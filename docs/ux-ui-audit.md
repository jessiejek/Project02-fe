...
**Follow-up pass (same day) — both remaining items addressed:**

- **Cross-day unpaid collection — fixed.** Payments now also queries `/api/bookings/staff/for-payment` (the previously-unused endpoint) as a supplement to the queue board. Testing it live immediately surfaced why it had never been wired up: its first response included a Checked-In and an In-Progress visit alongside genuinely completed ones — it is not trustworthy on its own. The fix is deliberately narrow: the supplement only contributes bookings that are already `status === "Completed"`, from a date strictly before today, and not already present in the queue board. Today's data stays 100% owned by the (verified-correct) queue board; the unreliable endpoint can only ever add noise to older days, never corrupt today's real collection queue.
- **Admin Dashboard's "Unpaid Completed" stat — investigated, no discrepancy found.** Checked `GET /api/reports/unpaid-completed-visits` live against the same test data used for the end-to-end walkthrough: it correctly reported `1` (matching Bianca Reyes), the same count Staff Dashboard and Staff Payments now show. No divergence reproduced — leaving this surface unchanged since there's no evidence of a bug, only unverified suspicion. Worth re-checking if a future report ever disagrees again, but not proactively "fixing" something that currently agrees.
- **`staff/bookings/[id]/page.tsx`** was left unchanged — direct by-ID reads are architecturally lower-risk than the filtered-list endpoints that caused this whole chain of bugs, and no problem was observed there.
