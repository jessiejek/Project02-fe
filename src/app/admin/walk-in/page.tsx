...
// Stitch walk_in_wizard_admin — same 3-step pattern as staff's walk-in, but
// the date is locked to today (per admin.md §4).
export default function AdminWalkInPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [allPatients, allDoctors, dayStatusRes] = await Promise.all([
      ...
      <AppShell role="admin">
        <Card className="mx-auto max-w-[26rem] text-center">
          ...
          <div className="flex gap-md">
            ...
            {createdBookingId && (
              <Link href={`/admin/bookings/${createdBookingId}`} className="flex-1">
      ...
      <div className="mx-auto max-w-[40rem]">
          ...
          <Card>
            ...
            <div className="mb-lg space-y-sm">
              ...
              {filteredPatients.map((p) => (
                <button
                  ...
                  className={cn("w-full rounded-lg border p-md text-left", patientId === p.id ? "border-primary ring-2 ring-primary/20" : "border-outline-variant")}
                >
            ...
            {quickRegisterOpen && (
              <div className="mb-lg grid grid-cols-1 gap-md rounded-lg border border-outline-variant p-md sm:grid-cols-2">
                ...
                />
                <Button onClick={handleQuickRegister} disabled={!canQuickRegister || registering} className="sm:col-span-2">
                  {registering ? "Registering..." : "Register Patient"}
                </Button>
              </div>
            )}
            <Button disabled={!patientId} onClick={() => setStep(2)} className="w-full">Continue</Button>
          </Card>
        )}
        ...
        {step === 2 && (
          <Card>
            ...
            <select value={doctorId ?? ""} onChange={(e) => setDoctorId(e.target.value)} className="mb-md w-full rounded-lg border border-outline-variant px-md py-sm">
            ...
            <div className="mb-md flex flex-wrap items-center gap-sm rounded-lg bg-surface-container-low px-md py-sm text-label-md text-on-surface-variant">
              <Icon name="lock" className="text-[16px]" />
              <span className="flex-1">Admin walk-ins are same-day only</span>
              <span className="rounded-lg border border-outline-variant px-sm py-xs">{today}</span>
            </div>
            <div className="mb-lg grid grid-cols-2 gap-md sm:grid-cols-4">
              {SLOTS.map((s) => (
                <button key={s} type="button" onClick={() => setSlot(s)} className={cn("rounded-lg border p-md text-label-md", slot === s ? "border-primary bg-primary text-on-primary" : "border-outline-variant")}>
                  {s}
                </button>
              ))}
            </div>
            <div className="flex gap-md">
              ...
              <Button disabled={!doctor || !slot} onClick={() => setStep(3)} className="flex-1">Continue</Button>
            </div>
          </Card>
        )}
        ...
        {step === 3 && patient && doctor && (
          <Card>
            <h2 className="mb-md text-headline-md text-on-surface">Review &amp; Create</h2>
            <div className="mb-lg space-y-sm rounded-lg bg-surface-container-low p-md text-body-md">
              ...
              <p className="flex items-center gap-sm"><strong>Payment:</strong> <span className="rounded-full bg-surface-container-high px-sm py-xs text-label-sm">Pay at Clinic (fixed)</span></p>
            </div>
            ...
            <div className="flex gap-md">
              ...
              <Button onClick={handleCreateBooking} disabled={creating} className="flex-1">{creating ? "Creating..." : "Create Booking"}</Button>
            </div>
          </Card>
        )}
      </div>
