...
export default async function BrowseDoctorsPage() {
  ...
  const [doctorsRes, ratingsRes, dayStatusRes] = await Promise.all([
  ...
  const doctors = (doctorsRes.data ?? []).filter((d) => one(d.staff_accounts)?.status !== "Inactive");
  ...
  const cards = doctors.map((doc) => {
    ...
    return {
      id: doc.doctor_id,
      name: one(doc.staff_accounts)?.full_name ?? "",
